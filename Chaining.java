...
public class Chaining {
	...
	/**Implements the hash function h(k)* ((A · k) mod 2w) */
	public int chain (int key) {return ((A*key) % (power2(w))) >> (w-r);}

	/**Inserts key k into hash table. Returns the number of collisions encountered*/
	public int insertKey(int key) {
		Table.get(chain(key)).add(key); // add the key at the end
		return Table.get(chain(key)).size();  // size of the secondary array is the amount of collisions
	}
